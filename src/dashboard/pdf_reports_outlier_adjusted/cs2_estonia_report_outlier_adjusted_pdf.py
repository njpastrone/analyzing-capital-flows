"""
Outlier-Adjusted Capital Flows Analysis - Case Study 2: Estonia Euro Adoption Report

This Streamlit application provides an exact mirror of the Estonia tab using winsorized data,
optimized for clean PDF export with professional formatting.

Research Focus: Estonia Euro Adoption Analysis - Capital Flow Volatility Before and After Euro Adoption (2011)
...
# Add parent directory (main dashboard) and outlier_adjusted_reports directory to path
current_file_dir = Path(__file__).parent.absolute()
parent_dir = current_file_dir.parent
outlier_adjusted_dir = parent_dir / 'outlier_adjusted_reports'
if str(parent_dir) not in sys.path:
    sys.path.insert(0, str(parent_dir))
if str(outlier_adjusted_dir) not in sys.path:
    sys.path.insert(0, str(outlier_adjusted_dir))
...
def main():
    ...
    st.title("🛡️ 🇪🇪 Estonia Euro Adoption Analysis (Outlier-Adjusted)")
    ...
    **Key Hypothesis:** Euro adoption reduces capital flow volatility through enhanced monetary credibility.
    ...
    - **Post-Euro Period:** 2011-2025 (full series)
...
def show_estonia_overall_analysis(include_crisis_years=True):
    ...
    try:
        ...
        # PDF Report: Disable download buttons by monkey-patching st.download_button
        original_download_button = st.download_button
        def disabled_download_button(*args, **kwargs):
            pass  # Do nothing - suppress download buttons for PDF export
        st.download_button = disabled_download_button
        
        # PDF Report: Disable expanders by monkey-patching st.expander  
        original_expander = st.expander
        def disabled_expander(*args, **kwargs):
        ...
        st.expander = disabled_expander
        
        # PDF Report: Disable st.button for HTML report generation
