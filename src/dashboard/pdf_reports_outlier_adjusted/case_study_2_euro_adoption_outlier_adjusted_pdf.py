"""
Case Study 2: Euro Adoption Impact Analysis - Baltic Countries (Outlier-Adjusted)
Temporal comparison of capital flow volatility before and after Euro adoption using winsorized data
...
import cs1_report_outlier_adjusted_pdf
from cs1_report_outlier_adjusted_pdf import (
...
def show_indicator_level_analysis_cs2(selected_country, include_crisis_years=True):
    ...
    **{selected_display_country} Analysis ({period_label}):** Euro adoption on {country_info['adoption_date']}
    ...
    with col2:
        # Individual standard deviation plot for PDF display
    ...
    **Summary:** Statistics for all {len(sorted_indicators)} capital flow indicators comparing pre and post Euro adoption periods.
    ...
    # Create individual figures for PDF display (matching CS1)
    n_cols_download = 3
    ...
    for batch_idx in range(0, n_indicators, n_cols_download):
        ...
        for col_idx, indicator in enumerate(batch_indicators):
            ...
            # Chart displayed for PDF export
            plt.close(fig_ind)
