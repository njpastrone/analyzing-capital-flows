...
def get_investment_type_order(indicator_name):
    """Get sort order for investment type indicators"""
    # Define order for investment types
    order_map = {
        'Direct': 1,
        'Portfolio': 2,
        'Other': 3,
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    ...
    title = "🇮🇸 Comparative Analysis: Iceland and SOEs"
    if data_type == "winsorized":
    ...
    **Key Hypothesis:** Iceland and other small open economies may exhibit different capital flow volatility patterns despite similar economic structures and currency regimes.
    ...
    case_study_3_main(data_type, ui_config, data_config, context)
    ...
    case_study_3_main_crisis_excluded(data_type, ui_config, data_config, context)

def case_study_3_main(data_type, ui_config, data_config, context="standalone"):
...
def case_study_3_main_crisis_excluded(data_type, ui_config, data_config, context="standalone"):
    ...
    **Enhanced Analysis:** Comparing Iceland to each individual SOE separately,
    ...
    display_results['Clean_Indicator'] = display_results['Indicator'].apply(get_indicator_nickname)
