"""
Capital Flows Analysis - Case Study 2: Estonia Euro Adoption Report

This Streamlit application provides an exact mirror of the Estonia tab from the main dashboard,
optimized for clean PDF export with professional formatting.

Research Focus: Estonia Euro Adoption Analysis - Capital Flow Volatility Before and After Euro Adoption (country_config['year'])
"""
...
COUNTRY_CONFIG = {
    'Estonia': {'year': 2011, 'flag': '🇪🇪', 'full_name': 'Estonia, Republic of'},
    'Latvia': {'year': 2014, 'flag': '🇱🇻', 'full_name': 'Latvia, Republic of'},
    'Lithuania': {'year': 2015, 'flag': '🇱🇹', 'full_name': 'Lithuania, Republic of'}
}

def get_data_configuration(data_type="full"):
    """Get data configuration for CS2"""
...
def main(country="Estonia", data_type="full", output_mode="interactive", context="standalone"):
    ...
    st.title(f"{country_config['flag']} Case Study 2: {country} Euro Adoption ({country_config['year']}) Analysis" + (" (Outlier-Adjusted)" if data_type == "winsorized" else ""))
    st.subheader("Capital Flow Volatility Before and After Euro Adoption (country_config['year'])")
    ...
    **Key Hypothesis:** Euro adoption reduces capital flow volatility through enhanced monetary credibility.
    ...
    with st.expander("📋 Data and Methodology", expanded=False):
        ...
        - **Euro Adoption Date:** January 1, country_config['year']
        - **Pre-Euro Period:** 1999-2010 (full series) 
        - **Post-Euro Period:** country_config['year']-2025 (full series)
    ...
    show_estonia_overall_analysis(country, include_crisis_years=True)

    # Indicator-Level Analysis
    st.subheader("🔍 Indicator-Level Analysis")
    show_estonia_indicator_analysis(country, include_crisis_years=True)
    ...
    st.subheader("📈 Overall Capital Flows Analysis")
    show_estonia_overall_analysis(country, include_crisis_years=False)

    # Indicator-Level Analysis - Crisis Excluded
    st.subheader("🔍 Indicator-Level Analysis")
    show_estonia_indicator_analysis(country, include_crisis_years=False)

def show_estonia_overall_analysis(country, include_crisis_years=True):
    """Show Estonia overall capital flows analysis with PDF-optimized formatting"""
    try:
        import sys
        from pathlib import Path
        import os
        ...
        with plt.style.context('default'):
            ...
            # Monkey-patch plt.subplots to enforce PDF-friendly figure sizes and titles
            ...
            try:
                country_full_name = COUNTRY_CONFIG[country]['full_name']
                show_overall_capital_flows_analysis_cs2(country_full_name, country, include_crisis_years)
            finally:
        ...
        st.error(f"Error displaying Estonia overall analysis: {str(e)}")

def show_estonia_indicator_analysis(country, include_crisis_years=True):
    """Show Estonia indicator-level analysis with PDF-optimized formatting"""
    try:
        import sys
        from pathlib import Path
        import os
        ...
        with plt.style.context('default'):
            ...
            try:
                country_full_name = COUNTRY_CONFIG[country]['full_name']
                show_indicator_level_analysis_cs2(country_full_name, include_crisis_years)
            finally:
                # Restore original settings and subplots function
                plt.subplots = original_subplots
                plt.rcParams.update(original_rcParams)
        ...
        st.error(f"Error displaying Estonia indicator analysis: {str(e)}")
