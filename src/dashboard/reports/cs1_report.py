...
def get_data_configuration(data_type="full"):
    ...
    -------
    dict
        Configuration with analysis_type and any other data settings
    """
    return {
        'analysis_type': 'winsorized' if data_type == 'winsorized' else 'full',
        'data_label': 'Outlier-Adjusted' if data_type == 'winsorized' else 'Full'
    }
...
def configure_ui_elements(output_mode="interactive"):
    ...
    ----------
    output_mode : str
        'interactive' for normal Streamlit, 'pdf' for PDF export mode
    ...
    if output_mode == "pdf":
        return {
            ...
            'show_interactive_elements': False
        }
    else:
        return {
            'use_expanders': True,
...
def load_default_data(include_crisis_years=True, data_type="full"):
    """
    ...
    try:
        ...
        # Use centralized data loading for CS1
        return _load_cs_data(
            case_study=1,
            analysis_type=data_config['analysis_type'],
...
def load_overall_capital_flows_data(include_crisis_years=True):
    ...
    """
    try:
        # Use centralized data loading for overall capital flows
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    """
    Main report application with parameterized data source and UI mode.

    Parameters
    ----------
    data_type : str, default="full"
        'full' for raw data, 'winsorized' for outlier-adjusted
    output_mode : str, default="interactive"
        'interactive' for normal Streamlit, 'pdf' for PDF export mode
    context : str, default="standalone"
        Execution context ('standalone' or 'main_app')
    """
    ...
    subtitle = f"Case Study 1: Iceland vs. Eurozone Comparison ({data_config['data_label']} Data)"
    ...
    **Hypothesis:** Iceland's capital flows show more volatility than the Eurozone bloc average
    ...
    if ui_config['use_expanders']:
        with st.expander("📋 Data and Methodology", expanded=ui_config['expanded_by_default']):
            st.markdown("""
            ### Data Sources
    ...
    **CV Ratio Summary (Iceland/Eurozone):**
    ...
    if ui_config['use_expanders']:
        with st.expander("📥 Download Individual Time Series Charts", expanded=False):
            time_series_figures = []
            n_cols_download = 3
    ...
    case_study_1_main_crisis_excluded(context=context, data_type=data_type, ui_config=ui_config)
...
def case_study_1_main_crisis_excluded(context="standalone", data_type="full", ui_config=None):
    """Main Case Study 1 application with crisis periods excluded"""
    final_data, analysis_indicators, metadata = load_default_data(
        include_crisis_years=False,
        data_type=data_type
    )
    ...
    **Individual Country Analysis Summary (Crisis-Excluded):**
    ...
    # Create individual figures for detailed downloads
    if ui_config and ui_config.get('use_expanders', True):
        with st.expander("📥 Download Individual Time Series Charts (Crisis-Excluded)", expanded=False):
            n_cols_download = 3
