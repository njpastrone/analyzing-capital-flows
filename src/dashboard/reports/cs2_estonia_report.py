...
This report analyzes Estonia's capital flow volatility before and after Euro adoption (2011).
...
# Estonia-specific configuration
...
def get_data_configuration(data_type="full"):
    """Get data configuration for Estonia analysis"""
...
def show_estonia_overall_analysis(data_config, ui_config, include_crisis_years=True):
    """Display Overall Capital Flows Analysis for Estonia"""
...
def show_estonia_indicator_analysis(data_config, ui_config, include_crisis_years=True):
    """Display Indicator-Level Analysis for Estonia"""
    study_version = "Full Series" if include_crisis_years else "Crisis-Excluded"
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    """Estonia Euro Adoption Analysis - Main Function"""
    ...
    if context == "standalone":
        st.set_page_config(
            page_title=f"CS2: Estonia Euro Adoption Analysis",
            page_icon="🇪🇪",
            layout="wide"
        )
    ...
    st.title(f"{COUNTRY_FLAG} Case Study 2: Estonia Euro Adoption ({EURO_ADOPTION_YEAR}) Analysis{title_suffix}")
    ...
    show_estonia_overall_analysis(data_config, ui_config, include_crisis_years=True)

    # Indicator-Level Analysis
    st.subheader("🔍 Indicator-Level Analysis")
    show_estonia_indicator_analysis(data_config, ui_config, include_crisis_years=True)
    ...
    st.subheader("📈 Overall Capital Flows Analysis")
    show_estonia_overall_analysis(data_config, ui_config, include_crisis_years=False)

    # Indicator-Level Analysis - Crisis Excluded
    st.subheader("🔍 Indicator-Level Analysis")
    show_estonia_indicator_analysis(data_config, ui_config, include_crisis_years=False)
