...
This report analyzes Latvia's capital flow volatility before and after Euro adoption (2014).
...
def get_data_configuration(data_type="full"):
    """Get data configuration for Latvia analysis"""
...
def show_latvia_overall_analysis(data_config, ui_config, include_crisis_years=True):
    """Display Overall Capital Flows Analysis for Latvia"""
...
def show_latvia_indicator_analysis(data_config, ui_config, include_crisis_years=True):
    """Display Indicator-Level Analysis for Latvia"""
    study_version = "Full Series" if include_crisis_years else "Crisis-Excluded"
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    """Latvia Euro Adoption Analysis - Main Function"""
    ...
    if context == "standalone":
        st.set_page_config(
            page_title=f"CS2: Latvia Euro Adoption Analysis",
            page_icon="🇱🇻",
            layout="wide"
        )
    ...
    st.title(f"{COUNTRY_FLAG} Case Study 2: Latvia Euro Adoption ({EURO_ADOPTION_YEAR}) Analysis{title_suffix}")
    ...
    show_latvia_overall_analysis(data_config, ui_config, include_crisis_years=True)

    # Indicator-Level Analysis
    st.subheader("🔍 Indicator-Level Analysis")
    show_latvia_indicator_analysis(data_config, ui_config, include_crisis_years=True)
    ...
    st.subheader("📈 Overall Capital Flows Analysis")
    show_latvia_overall_analysis(data_config, ui_config, include_crisis_years=False)

    # Indicator-Level Analysis - Crisis Excluded
    st.subheader("🔍 Indicator-Level Analysis")
    show_latvia_indicator_analysis(data_config, ui_config, include_crisis_years=False)
...
def display_methodology():
    ...
    - **Latvian Banking Crisis (2011-2012):** Additional crisis period unique to Latvia
    - **Extended Pre-Euro Period:** Later adoption allows for more pre-Euro data
    """)
