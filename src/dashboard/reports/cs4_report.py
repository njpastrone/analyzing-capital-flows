...
def get_data_configuration(data_type="full"):
    """Returns configuration dict with correct data paths and analysis type."""
    if data_type == "winsorized":
        return {
            'analysis_type': 'winsorized',
            'data_label': 'Outlier-Adjusted',
            'data_path_suffix': '_winsorized'
        }
    else:
        return {
            'analysis_type': 'full',
...
# Call: apply_professional_styling('cs4')
...
def display_methodology_section(ui_config):
    ...
    if ui_config['use_expanders']:
        ...
        pass  # Content will be shown without expander

def run_cs4_integrated_analysis(data_type, ui_config, data_config):
    ...
    # Initialize analysis framework with appropriate data path
    from pathlib import Path
    if data_type == "winsorized":
        data_dir = Path(__file__).parent.parent.parent.parent / "updated_data" / "Clean" / "CS4_Statistical_Modeling_winsorized"
    else:
        data_dir = Path(__file__).parent.parent.parent.parent / "updated_data" / "Clean" / "CS4_Statistical_Modeling"

    framework = CS4AnalysisFramework(data_dir=data_dir)
...
def display_comprehensive_analysis_overview(full_results, crisis_results, ui_config):
...
def display_summary_insights_and_export(full_results, crisis_results, ui_config):
...
def create_integrated_table(indicator, full_table, crisis_table, table_type, ui_config):
...
# NOTE: The following code was orphaned and has been commented out
# It appears to be code that should be inside a function but was accidentally
# left at module level. This was causing "ui_config not defined" errors.
"""
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    """Main application function with parameterized data source and UI mode."""
    ...
    title = "🇮🇸 Case Study 4: Comprehensive Statistical Analysis"
    if data_type == "winsorized":
        title += " (Outlier-Adjusted)"
    st.title(title)
    st.markdown("""
    **Objective:** Evaluate currency regime effects on capital flow volatility through comprehensive 
    ...
    with tab1:
        ...
        run_cs4_integrated_analysis(data_type, ui_config, data_config)
...
if __name__ == "__main__":
    ...
    main(data_type="full", output_mode="interactive", context="standalone")