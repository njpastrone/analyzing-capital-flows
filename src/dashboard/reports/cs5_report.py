...
def get_data_configuration(data_type="full"):
    ...
    if data_type == "winsorized":
        return {
            ...
            'controls_path': 'CS5_Capital_Controls_winsorized',
            'regimes_path': 'CS5_Regime_Analysis_winsorized'
        }
    else:
        return {
            'analysis_type': 'full',
            'data_label': 'Full',
            'controls_path': 'CS5_Capital_Controls',
            'regimes_path': 'CS5_Regime_Analysis'
        }
...
def load_capital_controls_data(data_type="full"):
    """Load capital controls analysis data"""
    data_config = get_data_configuration(data_type)
    data_paths = get_data_paths()

    # Use parameterized path based on data type
    if data_type == "winsorized":
        base_path = data_paths['clean_data'] / data_config['controls_path']
    else:
...
def load_regime_analysis_data(data_type="full"):
    ...
    # Use parameterized path based on data type
    if data_type == "winsorized":
        base_path = data_paths['clean_data'] / data_config['regimes_path']
    else:
...
def run_cs5_analysis(data_type="full", output_mode="interactive"):
    ...
    # Add data type indicator to title if winsorized
    title = "🌐 Case Study 5: Capital Controls and Exchange Rate Regime Analysis"
    if data_type == "winsorized":
    ...
    with st.spinner("📂 Loading capital controls data (Fernández et al., 1999-2017)..."):
        cc_data = load_capital_controls_data(data_type)
    ...
    **Methodological Notes:**
    ...
    with st.spinner("📂 Loading exchange rate regime classifications (IRR, 1999-2019)..."):
        regime_data = load_regime_analysis_data(data_type)
...
def main(data_type="full", output_mode="interactive", context="standalone"):
    """Main function with tabs and parameterized data source and UI mode."""
    ...
    # Only show tabs in interactive mode
    if output_mode == "interactive":
        tab1, tab2 = st.tabs(["📊 Analysis", "📚 Methodology"])

        with tab1:
            run_cs5_analysis(data_type, output_mode)
