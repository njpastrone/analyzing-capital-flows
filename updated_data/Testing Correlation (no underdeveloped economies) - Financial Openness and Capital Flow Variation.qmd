...
```{r}
# Set thresholds
gdp_threshold       <- 2e10   # e.g., $20B USD
gdp_threshold_year  <- 1999   # reference year to evaluate country GDP

# Countries that meet the GDP threshold in the reference year
keep_countries <- gdp_data_USD |>
  filter(TIME_PERIOD == gdp_threshold_year) |>
  filter(!is.na(`Gross domestic product (GDP), Current prices, US dollar`)) |>
  filter(`Gross domestic product (GDP), Current prices, US dollar` >= gdp_threshold) |>
  distinct(COUNTRY) |>
  pull(COUNTRY)

# Re-run pipeline: keep full panel for qualifying countries
net_flows_data_USD |>
...
```{r}
country_sd_flows_cap_controls_no_outliers |> distinct(iso3c) |> arrange(iso3c)
```
...
make_scatter_with_caption <- function(df, xcol, ycol, title, base_caption) {
  ...
  ggplot(df, aes(x = .data[[xcol]], y = .data[[ycol]])) +
    ...
    labs(
      ...
      title = title,
      subtitle = paste0("Countries with > ", formatC(gdp_threshold, format = "f", big.mark = ",", digits = 0), " USD 1999 GDP "),
