...
```{r}
net_flows_data_USD |> 
  filter(COUNTRY %in% c(eurozone_countries, soe_countries, baltics_countries, iceland)) |> 
  mutate(
...
net_flows_data_filtered_usd |>
  ...
  select(COUNTRY, YEAR, `Gross domestic product (GDP), Current prices, US dollar`, net_capital_flows_usd, country_group) |>
...
clean_net_flows_iso |> 
  ...
  group_by(iso3c, YEAR, country_group) |> 
  summarise(
...
make_scatter_with_caption <- function(df, xcol, ycol, title) { 
  ...
  p_val <- ifelse(ct$p.value < 0.001, "< 0.001", 
                  signif(ct$p.value, 3)) 
  df <- df %>% mutate( is_iceland = toupper(trimws(iso3c)) == "ISL", legend_lab = ifelse(is_iceland, "Iceland", "Other countries"))
  ggplot(df, aes(x = .data[[xcol]], y = .data[[ycol]])) + 
    ...
    labs( x = "Standard Deviation of Net Capital Flows (% GDP)", y = "Mean Overall Restrictions Index", title = title, subtitle = "Eurozone, Small Open Economies, Baltics, and Iceland", caption = paste0(" Cor = ", cor_val, " (p ", p_val, ")"), shape = NULL, color = NULL ) + theme_minimal() }
...
ADD COUNTRY GROUPS? SHOW EUROZONE VS SOEs, etc??
