...
# Helper: GDP-weighted mean (ignores NA and nonpositive weights)
wavg <- function(x, w) {
...
# Core builder: returns a wide table like your reference (YEAR, QUARTER, UNIT, iceland_pgdp, <regime>_pgdp_weighted, <regime>_pgdp_simple)
make_regime_table <- function(df, metric, case_country = "Iceland") {
  ...
  # Per-regime simple & weighted per quarter
  per_regime <- df %>%
    group_by(YEAR, QUARTER, regime_simple) %>%
    summarise(
      pgdp_simple   = mean(!!metric, na.rm = TRUE),
      pgdp_weighted = wavg(!!metric, gdp_weight_usd),
      .groups = "drop"
    )

  # Map regimes to slugs to create final column names
  reg_map <- tibble(regime_simple = factor(regime_levels, levels = regime_levels, ordered = TRUE),
                    slug = regime_slugs)

  per_regime <- per_regime %>% left_join(reg_map, by = "regime_simple")

  # Two wide frames: weighted and simple
  wide_w <- per_regime %>%
  ...
  # Column order: YEAR, QUARTER, UNIT, iceland_pgdp, then (for each regime) weighted, simple
  pairwise_cols <- as.vector(rbind(
    paste0(regime_slugs, "_pgdp_weighted"),
    paste0(regime_slugs, "_pgdp_simple")
  ))

  # Include only columns that actually exist (in case some regimes are absent in window)
...
# Build one table per indicator (add epi_pgdp / dpi_pgdp here if available)
