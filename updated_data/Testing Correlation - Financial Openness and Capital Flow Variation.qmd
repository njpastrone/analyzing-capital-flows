---
title: "Testing Correlation - Financial Openness and Capital Flow Variation"
format: html
editor: visual
---
...
### Drop Outliers and Re-run Correlation Test

```{r}
drop_outliers_pct <- function(df, col, lower = 0.01, upper = 0.99) {
  q_low <- quantile(df[[col]], lower, na.rm = TRUE)
  q_high <- quantile(df[[col]], upper, na.rm = TRUE)
  
  df |> filter(df[[col]] >= q_low & df[[col]] <= q_high)
}

```

```{r}
drop_outliers_pct(country_sd_flows_cap_controls, "country_sd_net_capital_flows_pgdp") -> country_sd_flows_cap_controls_no_outliers
```

```{r}
drop_outliers_pct(yearly_sd_flows_cap_controls, "yearly_sd_net_capital_flows_pgdp") -> yearly_sd_flows_cap_controls_no_outliers
```

```{r}
country_sd_flows_cap_controls_no_outliers |> distinct(COUNTRY) |> arrange(COUNTRY)
```
...
```{r}
cor.test(yearly_sd_flows_cap_controls_no_outliers$yearly_sd_net_capital_flows_pgdp, yearly_sd_flows_cap_controls_no_outliers$mean_overall_restrictions_index)
```

##### Country Aggregate SDs

```{r}
cor.test(country_sd_flows_cap_controls_no_outliers$country_sd_net_capital_flows_pgdp, country_sd_flows_cap_controls_no_outliers$mean_overall_restrictions_index)
```
...
# Helper function
make_scatter_with_caption <- function(df, xcol, ycol, title, base_caption) {
  # correlation + p-value
  ct <- cor.test(df[[xcol]], df[[ycol]])
  cor_val <- round(unname(ct$estimate), 2)
  p_val <- ifelse(ct$p.value < 0.001, "< 0.001", signif(ct$p.value, 3))
  
  # legend mapping (Iceland vs others)
  df <- df %>%
    mutate(
      is_iceland = toupper(trimws(COUNTRY)) == "Iceland",
  ...
  ggplot(df, aes(x = .data[[xcol]], y = .data[[ycol]])) +
    ...
    labs(
      ...
      title = title,
      caption = paste0(base_caption, "  Cor = ", cor_val, " (p ", p_val, ")"),
      shape = NULL,
      color = NULL
    ) +
    theme_minimal()
...
make_scatter_with_caption(
  ...
  base_caption = "Full dataset, no outliers removed."
)
```
...
```{r, message=F}
make_scatter_with_caption(
  yearly_sd_flows_cap_controls_no_outliers,
  xcol   = "yearly_sd_net_capital_flows_pgdp",
  ycol   = "mean_overall_restrictions_index",
  title  = "Yearly SD of Net Capital Flows vs. Restrictions Index",
  base_caption = "Outliers removed (1st & 99th percentile)."
)
```
...
```{r, message=F}
make_scatter_with_caption(
  country_sd_flows_cap_controls_no_outliers,
  xcol   = "country_sd_net_capital_flows_pgdp",
  ycol   = "mean_overall_restrictions_index",
  title  = "Country SD of Net Capital Flows vs. Restrictions Index",
  base_caption = "Outliers removed (1st & 99th percentile)."
...
```{r}
write_csv(yearly_sd_flows_cap_controls, "Clean/CS5_Capital_Controls/sd_yearly_flows.csv")
write_csv(yearly_sd_flows_cap_controls_no_outliers, "Clean/CS5_Capital_Controls/sd_yearly_flows_no_outliers.csv")
write_csv(country_sd_flows_cap_controls, "Clean/CS5_Capital_Controls/sd_country_flows.csv")
write_csv(country_sd_flows_cap_controls_no_outliers, "Clean/CS5_Capital_Controls/sd_country_flows_no_outliers.csv")
```
